(function ($) {
  ...
  function getCardTopPosition(startTime) {
    return (settings.lineHeight + settings.borderWidth) * ((startTime - settings.lowerHour)* 2);
  }
  ...
  */
  ...
  function appendTasks(placeholder, tasks) {
    ...
    for (var i = 0; i < args.args.length; i++) {
      var width = (settings.columnWidth - 6) / (args.args[i] || 1);
  ...
  */
  $.fn.skeduler = function (options) {
    ...
    for (var i = settings.lowerHour; i < settings.upperHour; i++) {
      // Populate timeline
      div.clone()
        .text(toTimeString(i))
